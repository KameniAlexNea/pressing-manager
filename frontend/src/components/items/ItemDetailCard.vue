    ...
    <a-skeleton :loading="loading" active>
      ...
      <div v-if="item.items && item.items.length" class="items-section">
        <div class="items-header">
          ...
          <a-button v-if="showActions && !isEditing && item.status !== 'delivered'"
            size="small"
            @click="startEditing"
            :disabled="loading">
            Modifier
      ...
      <a-space v-if="showActions" class="status-actions" :wrap="true">
        <a-button v-if="item.status === 'received'" type="primary" @click="$emit('statusChange', item.id, 'cleaned')"
          :loading="loading" :disabled="loading">
          Marquer comme Nettoyé
        </a-button>
        <a-button v-if="item.status === 'cleaned'" type="primary" @click="$emit('statusChange', item.id, 'delivered')"
          :loading="loading" :disabled="loading">
          Marquer comme Livré
        </a-button>
      </a-space>
...
import { updateItemsList, type ClothingItem, type ItemLine } from '../../store/items'
